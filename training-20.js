//! [Develop] training -20 | 20번 훈련기념을 축하한다며 다시돌아온 포켓몬 사장님
...
class Pokemon {
  ...
  //setter 메서드는 데이터 타입을 검사해 안정성을 높인다.
  set name(value){
...
// 데이터베이스 연결 정보 configuration "정의" 라는 단어를 줄여서 config라고 부른다 
...
const dbConfig = {
  ...
  password : 'kkai0114@@',
  database : 'POKEMON',
  port: 3306, // MySQL 의 일반적인 포트 3306
  connectionLimit :5, // 동시에 DB에 접속가능한 클라이언트 5개. 여기서 클라이언트는 개발자들이다.
  waitForConnections: true // 클라이언트가 연결을 요청할 때 대기 여부
};

//javascript-14예제의 5번 코드인 '객체를 변환하는 함수' 참고 하기 
...
//보통의 경우 
//class 를 선언한 코드와 JSON으로 변환하는 함수코드는 분리되어 있다.
//본 예제에서는 굳이 JSON으로 바꾸지 않아도 데이터베이스에 create(생성)되지만
//실무에서는 JSON 으로 내보내고 가져오는 경우가 많다.
//하나의 '객체'를 다른 '객체'로 변환하는 것을 '직렬화' 라고 한다.
//JSON 포멧은 다른 언어에서도 (파이썬 등) 범용성이 무궁무진하고 
// 분리되는 과정에서 자체가 서비스 측면에서는 보안적으로도 중요하다.
//Node.js 특성상 JSON.parse()메서드가 내장메서드이기 때문에 상당히 간편함.
...
function savePokemonJSONToDatabase(pokemonJSON) {
  ...
  connection.connect((err)=> {
    ...
    //쿼리문은 기본 구조가 '무엇을' '어디에' '어떻게' 이다.
    //영어의 어순을 그대로 따르는 것이 특징이다.
    //삽입하겠다 (insert into)
    //pokemon 이라는 테이블에, 열(column)은 name,type,level 세칸임.
    //그리고 그 옆에 열에 들어갈 데이터는 ? 로 표시한다.
...
// .query()
// .end();
// 위 세게의 메서드는 세트로 사용되는 경우가 많아,
// 마치 HTTP 모듈의 wirteHead , write , end 메서드와 꼭 닮았다.
// 이렇게 "묶음단위"로 실행되는 형태를 공학적 용어로 트랜잭션(transaction) 이라고 한다. 

const pikachu = new Pokemon('피카츄', '전기' , 10);
const pokemonJSON = convertPokemonToJSON(pikachu);
savePokemonJSONToDatabase(pokemonJSON);
